...
import org.sotap.MissionTap.Utils.Log;
        ...
        if (cmd.getName().equalsIgnoreCase("missiontap")) {
            ...
            switch (args[0]) {
                ...
                case "init": {
                    ...
                    sender.sendMessage(Log.translateColor(Log.INFO + "正在初始化当前设定所需数据..."));
                    ...
                    sender.sendMessage(Log.translateColor(Log.SUCCESS + "初始化完成。"));
                    break;
                }
                ...
                case "player": {
                    ...
                    if (args.length < 3) {
                        sender.sendMessage(Log.translateColor(Log.FAILED + "参数不足。"));
                        break;
                    }
                    ...
                    if (pl != null) {
                        ...
                        switch (args[2]) {
                            case "clear-submittion": {
                                if (args.length >= 4) {
                                    if (List.of("daily", "weekly").contains(args[3])) {
                                        ...
                                        sender.sendMessage(Log.translateColor(Log.SUCCESS + "成功清除 &a" + pl.getName() + " &r的任务提交记录。"));
                                    } else {
                                        sender.sendMessage(Log.translateColor(Log.FAILED + "Invalid argument."));
                                    }
                                } else {
                                    playerdata.set("submitted-list", null);
                                    sender.sendMessage(Log.translateColor(Log.SUCCESS + "成功清除 &a" + pl.getName() + "&r 的&e所有&r任务提交记录。"));
                                }
                            ...
                            default: {
                                sender.sendMessage(Log.translateColor(Log.FAILED + "无效参数。"));
                            }
                        }
                    } else {
                        sender.sendMessage(Log.translateColor(Log.FAILED + "指定的玩家&c不在线&r或者&c不存在&r。"));
                    }
                    break;
                }
                ...
                case "reload": {
                    ...
                    Functions.reloadPlugin(plugin);
                    sender.sendMessage(Log.translateColor(Log.SUCCESS + "成功重载配置文件。"));
                    break;
                }

                case "about": {
                    sender.sendMessage(Log.translateColor("&e# 关于 MissionTap"));
                    sender.sendMessage(Log.translateColor("&bMissionTap&r 是 SoTap 独立开发的第一个玩法性插件，如有问题请见谅 >_<!"));
                    sender.sendMessage(Log.translateColor("如果您对 MissionTap 有什么想说的，可以在讨论群内联系管理组，畅所欲言~"));
                    sender.sendMessage(Log.translateColor("GitHub: &a&nhttps://github.com/sotapmc/MissionTap"));
                    break;
                }
